...
logger = get_logger("legal_mcp_server")
...
try:
    ...
    logger.critical("Failed to load LegalDatabase", extra={"error": str(e)})
    raise e
...
def _search_maritime_precedents_internal(query: str, top_k: int = 3) -> Dict:
    ...
    logger.debug("Internal search called",
    ...
    try:
        ...
        logger.error("Internal search failed",
...
def search_maritime_precedents(query: str, top_k: int = 3) -> Dict:
    ...
    """
    logger.info("Tool 'search_maritime_precedents' invoked",
                extra={"query": query, "top_k": top_k})
    result = _search_maritime_precedents_internal(query, top_k)
    logger.info("Tool 'search_maritime_precedents' completed",
...
def get_liability_convention_article(article: str) -> Dict:
    ...
    """
    logger.info("Tool 'get_liability_convention_article' invoked",
    ...
    convention_articles = {
        ...
        "IV": {
            ...
            "text": (
                ...
                "object of States which have jointly launched a space object, they shall "
        ...
        "V": {
            ...
            "text": (
                "Whenever a launching State is exonerated wholly or partially from "
                "absolute liability, a launching State may also be exonerated wholly "
                "or partially from fault-based liability if it establishes that the "
                "damage resulted from gross negligence or from an act or omission done "
                "with intent to cause damage on the part of a claimant State."
            ),
    ...
    if article_upper in convention_articles:
        logger.info("Article found", extra={"article": article_upper})
        return {
            "success": True,
        ...
        logger.warning("Article not found",
                       extra={"article": article,
                               "available": list(convention_articles.keys())})
        return {
            "success":           False,
...
def analyze_liability_factors(
    ...
    Args:
        ...
        collision_occurred: Whether an actual collision/damage event occurred

    Returns:
        Dictionary with liability analysis and applicable doctrines
    """
    logger.info(
        "Tool 'analyze_liability_factors' invoked",
        extra={
            "obj1":      object_1_status,
            "obj2":      object_2_status,
            "warning":   warning_provided,
            "maneuver":  maneuver_possible,
            "collision": collision_occurred,
        },
    )

    # ── No collision → no liability under Article III ─────────────────────
    # Article III requires proof of fault AND actual damage. Without a collision
    # there is no damage and therefore no cause of action exists.
    if not collision_occurred:
        logger.info("No collision — returning zero-liability analysis")
        return {
            "success": True,
            "applicable_doctrines": [],
            "liability_split": {"object_1": "0%", "object_2": "0%"},
            "reasoning": [
                "No collision occurred — no damage establishes no cause of action "
                "under Article III of the 1972 Liability Convention, which requires "
                "proof of fault AND actual damage."
            ],
    ...
    # ── Determine applicable doctrines (only what is factually relevant) ────
    ...
    if object_1_status == "ACTIVE" and object_2_status == "ACTIVE":
        if warning_provided and maneuver_possible:
            ...
            factors["applicable_doctrines"].append({
                "name":   "Negligent Navigation",
                "impact": "Active satellites share duty of care",
            })
    ...
    if object_1_status == "DRIFTING" and object_2_status == "ACTIVE":
        if warning_provided and maneuver_possible:
            factors["liability_split"] = {"object_1": "10-20%", "object_2": "80-90%"}
            factors["reasoning"].append(
                "Active object had last clear chance to avoid drifting debris"
            )
        else:
            factors["liability_split"] = {"object_1": "30-40%", "object_2": "60-70%"}
            factors["reasoning"].append(
        ...
        if warning_provided and maneuver_possible:
            factors["liability_split"] = {"object_1": "80-90%", "object_2": "10-20%"}
            factors["reasoning"].append(
                "Object 1 had capability and warning but failed to maneuver"
            )
        else:
            factors["liability_split"] = {"object_1": "60-70%", "object_2": "30-40%"}

    elif object_1_status == "ACTIVE" and object_2_status == "ACTIVE":
        factors["liability_split"] = {"object_1": "50%", "object_2": "50%"}
        factors["reasoning"].append(
...
def get_all_precedents() -> Dict:
    ...
    try:
        ...
        logger.info("Precedents retrieved", extra={"count": count})
        return {
            "success":          True,
        ...
        logger.error("Error retrieving precedents", extra={"error": str(e)})
...
def calculate_damages_estimate(
    ...
    """
    logger.info(
        "Tool 'calculate_damages_estimate' invoked",
        extra={
            "speed_m_s": collision_speed_m_s,
            "mass_kg":   object_mass_kg,
            "value_usd": mission_value_usd,
        },
    )
    ...
    logger.debug("Kinetic energy calculated",
                 extra={"kinetic_energy_mj": kinetic_energy_mj})
    ...
    logger.info("Damage assessment complete", extra={"severity": severity})
    ...
    if mission_value_usd:
        ...
        logger.info("Financial loss estimated",
                    extra={"estimated_loss_usd": round(estimated_loss, 2)})
