...
def suggest_valid_satellites() -> None:
...
def print_judgment(final_state: dict) -> None:
    ...
    print(f"   {obj1['sat_name']} (NORAD {obj1['norad_id']}): {fault1}% fault")
    print(f"   {obj2['sat_name']} (NORAD {obj2['norad_id']}): {fault2}% fault")
    ...
    print(f"Prompt version: {judgment.get('prompt_version', 'N/A')}")
    print("=" * 80 + "\n")


def save_judgment(final_state: dict, output_file: str) -> None:
...
Examples:
  ...
  python run_analysis.py --list-satellites
    ...
    if args.obj1 == args.obj2:
        parser.error("--obj1 and --obj2 must be different NORAD IDs")

    # Warn on high NORAD IDs
    ...
    # Load tools from registry (single source of truth)
    print("Loading MCP tools...")
    try:
        orbital_tools, legal_tools = load_tools()
        print(
            f"✓ Loaded {len(orbital_tools)} orbital tools "
            f"and {len(legal_tools)} legal tools"
        )
    ...
    try:
        # Run analysis — CLI uses asyncio.run() which delegates to the shared async path
